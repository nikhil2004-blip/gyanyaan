...
const MISSION_NAMES = {
  mangalyaan: "MANGALYAAN — MARS ORBITER MISSION",
...
const LEVELS = [
  { id: 1, name: "ASSEMBLY", description: "Build and configure the Mars Orbiter Mission rocket", difficulty: "BEGINNER", icon: "🔧" },
  { id: 2, name: "ASCENT", description: "Control the ascent phase and manage stage separations", difficulty: "INTERMEDIATE", icon: "🚀" },
  { id: 3, name: "INJECTION", description: "Execute precise orbital maneuvers to reach Mars orbit", difficulty: "ADVANCED", icon: "🛰️" },
  { id: 4, name: "ORBIT RAISER", description: "Raise orbit through strategic perigee burns for TMI", difficulty: "EXPERT", icon: "🌌" },
  { id: 5, name: "TRANS-MARS INJECTION", description: "Execute the critical burn to escape Earth and head to Mars", difficulty: "EXPERT", icon: "🔥" },
  { id: 6, name: "MARS TRANSFER", description: "Navigate deep space cruise and manage spacecraft systems", difficulty: "MASTER", icon: "🪐" },
  { id: 7, name: "MARS ORBIT INSERTION", description: "Execute the critical braking burn to capture into Mars orbit", difficulty: "MASTER", icon: "🔴" },
  { id: 8, name: "MARS SCIENCE OPS", description: "Operate scientific instruments and collect Mars data", difficulty: "MASTER", icon: "🔬" },
];
  ...
  >
    ...
    <div className="relative z-10 flex flex-col items-center gap-4">
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-slate-400 font-mono leading-relaxed max-w-xs">{level.description}</p>
      </div>
      <div className={`text-[10px] font-pixel px-3 py-1 border-2 ${isUnlocked ? "bg-blue-900/50 border-blue-500 text-blue-300" : "bg-slate-800/50 border-slate-600 text-slate-500"}`}>
        {level.difficulty}
      </div>
    </div>
...
export default function LevelSelection() {
  ...
  useEffect(() => {
    async function fetchProgress() {
      const progress = await getLevelProgress(missionId);
      setUnlockedLevels(progress?.unlockedLevels || [1]);
      setCompletedLevels(progress?.completedLevels || []);
      setLoading(false);
    }
    fetchProgress();
  }, [missionId, getLevelProgress]);
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-white via-neonBlue to-white text-glow">
              {MISSION_NAMES[missionId] || missionId.toUpperCase()}
            </h1>
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 md:grid-cols-3 gap-8">
          {LEVELS.map((level) => (
            <LevelCard
              ...
              onClick={() => navigate(`/missions/${missionId}/levels/${level.id}`)}
            />
          ))}
        </div>
      </div>
